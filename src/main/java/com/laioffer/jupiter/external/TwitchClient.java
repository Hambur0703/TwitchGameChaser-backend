...
public class TwitchClient {
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) {
            return String.format(url, limit); // when gameName is not given, we just get top games.
        } else {
            try {
    ...
    private String searchTwitch(String url) throws TwitchException {
        ...
        ResponseHandler<String> responseHandler = response -> {
            ...
            // transfer entity to a string, and store the string in a JSONObject
            JSONObject obj = new JSONObject(EntityUtils.toString(entity));
            // and then we take out "data" from JSONObject
            return obj.getJSONArray("data").toString();
        };

        // First, getting httpclient, setting Get's parameters, sending request,
        // and leaving the rest(return value after sending http request) to responseHandler
        try {
    ...
    private String buildSearchURL(String url, String gameId, int limit) throws TwitchException{
        try {
            ...
            e.printStackTrace();
            throw new TwitchException("Cannot encode game ID: " + gameId);
        }
    ...
    private List<Item> searchClips(String gameId, int limit) throws TwitchException {
        List<Item> clips = getItemList(
                searchTwitch(buildSearchURL(CLIP_SEARCH_URL_TEMPLATE, gameId, limit)));
        for (Item item : clips) {
    ...
    private List<Item> searchVideos(String gameId, int limit) throws TwitchException {
        List<Item> videos = getItemList(
                searchTwitch(buildSearchURL(VIDEO_SEARCH_URL_TEMPLATE, gameId, limit)));
