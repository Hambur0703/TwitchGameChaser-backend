...
public class ItemRecommender {
    ...
    private List<Item> recommendByFavoriteHistory(
        ...
        // Function.identity() means putting same id into the same group,
        // Collectors.counting() means collect each group's count.
        ...
        for (Map.Entry<String, Long> favoriteGame : sortedFavoriteGameIdListByCount) {
            ...
            for (Item item : items) {
                ...
                // optimized: if users have favorited the item, don't recommend the item again.
    ...
    // Return a map of Item objects as the recommendation result.(None registered user)
    // Keys of the may are [Stream, Video, Clip].
    // Each key is corresponding to a list of Items objects,
